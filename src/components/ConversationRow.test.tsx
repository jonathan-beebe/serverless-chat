...
// CR-008 / CR-009: Home owns the open-menu id and the LiveRegion. Row tests
// render this tiny harness so that lifted state still exists (menu toggle,
// copy-announcement surface) without pulling in `useConversations` /
// `useSession` / `useNavigate`. The 8-prop contract is exercised end-to-end.
...
function RowHarness({ record, isLive = false, onRename = () => {}, onDelete = () => {} }: HarnessProps) {
...
describe('ConversationRow rendering (FEAT-012 AC#18 / #20 / #21 / #26)', () => {
  it('renders a row per past conversation with the label and a Resume button', async () => {
    const recordA = await seed('aaa', { label: 'Lunch chat' })
    const recordB = await seed('bbb', { label: 'Project sync' })

    render(
      <MemoryRouter>
        <RowHarness record={recordA} />
        <RowHarness record={recordB} />
      </MemoryRouter>,
    )
  ...
  it('Resume is a link to the row`s canonical conversation URL (AC#26 / ARCH-001)', async () => {
    ...
    const record = await seed('aaa', { label: 'Lunch chat' })
    renderRow({ record })
  ...
  it('shows the last message body (truncated to ~50 chars) as the peek (AC#18)', async () => {
    const record = await seed('aaa', { label: 'With history' })
  ...
  it('Delete with confirm removes the row and the underlying record (AC#20)', async () => {
    const record = await seed('aaa', { label: 'Goodbye' })
    const onDelete = vi.fn()
    renderRow({ record, onDelete })
  ...
  it('Delete cancel leaves the row in place (AC#20)', async () => {
    const record = await seed('aaa', { label: 'Stays' })
    const onDelete = vi.fn()
    renderRow({ record, onDelete })
  ...
  it('Rename inline edits the row`s label (AC#21)', async () => {
    const record = await seed('aaa', { label: 'Old name' })
    const onRename = vi.fn()
    renderRow({ record, onRename })
...
describe('ConversationRow menu dismissal (CR-008)', () => {
  it('closes the open menu on pointerdown outside the row', async () => {
    const record = await seed('aaa', { label: 'Row A' })
    render(
      <MemoryRouter>
        <h1>outside</h1>
        <RowHarness record={record} />
      </MemoryRouter>,
    )
    ...
    const heading = screen.getByRole('heading', { name: /outside/i })
  ...
  it('opening row B`s menu closes row A`s (single-open invariant)', async () => {
    // CR-008: the single-open invariant lives in Home (it owns
    // `openMenuId`). Mirror Home's contract here with one shared state
    // driving two rows; opening B sets A's `isMenuOpen` to false on the
    // same render.
    const recordA = await seed('aaa', { label: 'Row A' })
    const recordB = await seed('bbb', { label: 'Row B' })
      ...
      return (
        <ul>
          <ConversationRow
            ...
            onDelete={() => {}}
            onAnnounce={() => {}}
            isMenuOpen={openId === 'aaa'}
            onOpenMenu={() => setOpenId('aaa')}
          ...
          <ConversationRow
            ...
            onDelete={() => {}}
            onAnnounce={() => {}}
            isMenuOpen={openId === 'bbb'}
            onOpenMenu={() => setOpenId('bbb')}
            onCloseMenu={() => setOpenId(null)}
            isLive={false}
          />
        </ul>
...
describe('ConversationRow menu Copy transcript (CR-009)', () => {
  ...
  it('renders Copy transcript between Rename and Delete in the row menu', async () => {
    const record = await seedWithMessages('aaa', 1)
    renderRow({ record })
  ...
  it('Copy transcript writes the formatted (timestamped) markdown to the clipboard', async () => {
    const record = await seedWithMessages('aaa', 2)
  ...
  it('Copy transcript is disabled when the conversation has no messages', async () => {
    // The row's per-row preview-load resolves to `hasMessages: false` for
    // a conversation with no messages and disables Copy. Home's first-load
    // cull doesn't apply here — Row owns the disabled-menu branch.
    const record: ConversationRecord = {
      id: 'aaa',
      createdAt: Date.now() - 60_000,
      lastActivityAt: Date.now() - 60_000,
...
describe('ConversationRow menu APG keyboard navigation (A11Y-025)', () => {
  ...
  it('auto-focuses the first non-disabled menuitem on open (Rename)', async () => {
    const record = await seedRow('aaa', 'Row A')
    renderRow({ record })
  ...
  it('Copy transcript uses aria-disabled (not native disabled) and remains focusable when the row has no messages', async () => {
    const record = await seedRow('aaa', 'Empty', false)
    const writeText = vi.fn().mockResolvedValue(undefined)
